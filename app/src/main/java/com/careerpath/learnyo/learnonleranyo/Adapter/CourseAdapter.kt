...
class CourseAdapter (private val allCoursesList: ArrayList<Course>,context: Context): RecyclerView.Adapter<CourseAdapter.ImageViewHolder>() {

    val ctx: Context = context

    public class ImageViewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        val courseImage_singleCourse: ImageView = itemView.findViewById(R.id.courseimage)
        val CourseRating_singleCourse: RatingBar = itemView.findViewById(R.id.courserating)
        val CoursefullName_singlecourse: TextView = itemView.findViewById(R.id.courseName)
        val CourseCategory_singlecourse: TextView = itemView.findViewById(R.id.category)
...
//        return ImageViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.course_view, parent,false))
        ...
        val CourseView = LayoutInflater.from(parent.context).inflate(R.layout.course_view, parent,false)
    ...
    override fun getItemCount(): Int {
        ...
        val limit = 8
        return Math.min(allCoursesList.size, limit)
    }
    ...
    override fun onBindViewHolder(holder: ImageViewHolder, position: Int) {
        ...
        Glide.with(holder.itemView).load(course.imageUri).into(holder.itemView.courseimage)
